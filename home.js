import { generateBasedOnLocation } from './src/generateBy.ts';
...
async function loadCards() {
    try {
        const places = await generateBasedOnLocation();
        console.log('Fetched places:', places);  // Check if the array is fetched

        const cardStack = document.getElementById('cardStack');
        if (!cardStack) {
            console.error('No element with ID cardStack found');
            return;
        }
